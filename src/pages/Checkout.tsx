...
import { ArrowLeft, ArrowRight, Check, Loader2, Package, MapPin, CreditCard, AlertCircle, Truck, RefreshCw } from 'lucide-react';
...
const Checkout = () => {
  ...
  const prepareCheckoutData = () => {
    ...
    // Free shipping for subscription-only orders
    ...
    return {
      ...
      items: cartItems.map(item => ({
        ...
        original_price: item.original_price || item.product.price,
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            {item.is_subscription && (
                              <div className="mt-1 flex items-center gap-2">
                                <span className="inline-flex items-center gap-1 px-2 py-0.5 bg-primary/20 text-primary text-xs font-semibold rounded">
                  ...
                  {hasSubscriptionItems && (
                    <div className="bg-primary/5 border border-primary/30 rounded-xl p-6">
                      ...
                      <div className="space-y-3">
                        {cartItems.filter(item => item.is_subscription).map((item) => (
                          <div 
                            key={`sub-${item.product.id}-${item.weight}`}
                            className="flex justify-between text-sm"
                          >
                            <span>{item.product.name} ({item.weight}g) × {item.quantity}</span>
                            <span className="text-muted-foreground">Monthly delivery</span>
                          </div>
                        ))}
                      </div>
                      ...
                      <div className="mt-4 pt-4 border-t border-primary/20">
                        <ul className="space-y-2 text-sm">
                          <li className="flex items-center gap-2 text-green-600">
                            <Check className="w-4 h-4" />
                            <span className="font-medium">Free Shipping on subscription items</span>
                          </li>
                          ...
                          <li className="flex items-center gap-2 text-muted-foreground">
                            <Check className="w-4 h-4" />
                            <span>Cancel or pause anytime from your account</span>
                          </li>
                        </ul>
                      </div>
                      ...
                      <div className="mt-4 p-3 bg-primary/10 rounded-lg">
                        <p className="text-sm text-foreground">
                          <strong>Note:</strong> Your subscription will begin today. You'll be charged monthly for subscription items.
                        </p>
                      </div>
                    </div>
                  )}
                ...
                <div className="bg-card border border-border rounded-xl p-8 text-center">
                  ...
                  {hasSubscriptionItems && (
                    <div className="bg-primary/10 border border-primary/30 rounded-lg p-4 mb-4 text-sm">
                      <p className="font-medium text-primary mb-2 flex items-center justify-center gap-2">
