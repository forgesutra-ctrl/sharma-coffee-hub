...
import { ShoppingBag, Menu, X, User, ChevronDown, Search, Instagram, Facebook, Twitter, Phone, Heart } from 'lucide-react';
  ...
  { name: 'Home', href: '/' },
  {
    ...
    children: [
      { 
        ...
        items: [
          { name: 'Coorg Classic', href: '/shop/coorg-classic' },
          { name: 'Gold Blend', href: '/shop/gold-blend' },
          { name: 'Premium Blend', href: '/shop/premium-blend' },
        ]
      },
      { 
        ...
        items: [
          ...
          { name: 'Royal Caffeine', href: '/shop/royal-caffeine' },
        ]
      },
      { 
        name: 'More Products', 
        ...
        items: [
          { name: 'Instant Coffee', href: '/shop/instant-coffee-decoctions' },
          { name: 'Tea Collection', href: '/shop/tea-products' },
          { name: 'Accessories', href: '/shop/other-products' },
        ]
      },
    ],
  },
...
const announcements = [
  ...
  { text: 'FARM TO CUP', highlight: 'Direct from Coorg since 1983' },
  { text: '100% AUTHENTIC', highlight: 'Premium South Indian coffee' },
];
  ...
  return (
    <>
        ...
        <div className="max-w-7xl mx-auto px-4 flex items-center justify-between">
          ...
          <div className="flex-1 text-center overflow-hidden">
            <div 
              ...
              style={{ transform: `translateY(-${currentAnnouncement * 100}%)` }}
            >
              {announcements.map((ann, idx) => (
                <div 
                  key={idx}
                  className={cn(
                    "h-6 flex items-center justify-center gap-2 text-xs sm:text-sm tracking-wider transition-opacity duration-300",
                    idx === currentAnnouncement ? "opacity-100" : "opacity-0"
                  )}
                  style={{ display: idx === currentAnnouncement ? 'flex' : 'none' }}
                >
      ...
      >
        <div className="max-w-7xl mx-auto px-4">
          ...
          <div className="flex items-center justify-between h-16 lg:h-20">
            ...
            <Link to="/" className="flex-shrink-0 group flex flex-col items-center py-2">
              ...
              />
              <span className="hidden md:block text-[10px] tracking-[0.25em] text-muted-foreground uppercase mt-0.5">
                Est. 1983 • Coorg
              </span>
            </Link>
            ...
            <div className="flex items-center gap-2 flex-1 justify-end">
              <Link
                to="/brewing-guide"
                className="hidden sm:flex p-2.5 text-foreground/70 hover:text-primary hover:bg-muted/50 rounded-full transition-all"
                aria-label="Wishlist"
              >
              ...
              <Link 
                to="/cart" 
                className="relative p-2.5 text-foreground/70 hover:text-primary hover:bg-muted/50 rounded-full transition-all"
              >
                <ShoppingBag className="w-5 h-5" />
                {cartCount > 0 && (
                  <span className="absolute top-0 right-0 w-5 h-5 bg-primary text-primary-foreground text-[10px] font-bold rounded-full flex items-center justify-center animate-scale-in">
                    {cartCount}
                  </span>
                )}
              </Link>
          ...
          <div 
            className="hidden lg:flex items-center justify-center gap-8 py-3 border-t border-border/50" 
            ref={dropdownRef}
          >
            {navLinks.map((link) => (
              <div 
              ...
              >
                ...
                {link.children && activeDropdown === link.name && (
                  <div 
                    className="absolute top-full left-1/2 -translate-x-1/2 mt-2 w-[600px] bg-card border border-border rounded-lg shadow-2xl animate-fade-in z-50"
        ...
        <div
          className={cn(
            'lg:hidden fixed inset-x-0 top-[calc(2.5rem+4rem)] bottom-0 bg-background z-40 transform transition-all duration-500 ease-out overflow-y-auto',
          ...
          <div className="flex flex-col p-6 space-y-1 min-h-full">
            ...
            <div className="pt-6 mt-auto border-t border-border/30">
              ...
              >
                ...
                Shopping Bag
                {cartCount > 0 && (
                  <span className="ml-auto px-2 py-0.5 bg-primary text-primary-foreground text-xs font-medium rounded-full">
                    {cartCount}
                  </span>
                )}
              </Link>
