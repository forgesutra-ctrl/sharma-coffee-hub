...
export default function AIChatBot() {
  ...
  const handleSend = async () => {
    ...
    try {
      await streamChat(newMessages, chunk => updateAssistant(chunk), () => setIsLoading(false));
    } catch (error) {
  ...
  const quickQuestions = ["What's your best-selling coffee?", "How do I brew filter coffee?", "Do you ship internationally?", "What's the difference between your blends?"];
  return <>
      {/* Chat Toggle Button */}
      <motion.button onClick={() => setIsOpen(true)} className={`fixed bottom-6 right-6 z-50 ${isOpen ? 'hidden' : 'block'}`} initial={{
      scale: 0,
      opacity: 0
    }} animate={{
      scale: 1,
        ...
        duration: 0.2
            ...
            <div className="bg-primary text-primary-foreground p-4 flex items-center justify-between shrink-0">
              ...
              <div className="flex items-center gap-1">
                {messages.length > 0 && <button onClick={clearHistory} className="w-8 h-8 rounded-full hover:bg-primary-foreground/20 flex items-center justify-center transition-colors" aria-label="Clear chat history" title="Clear chat">
                    <Trash2 className="w-4 h-4" />
                  </button>}
                <button onClick={() => setIsOpen(false)} className="w-8 h-8 rounded-full hover:bg-primary-foreground/20 flex items-center justify-center transition-colors" aria-label="Close chat">
                  <X className="w-5 h-5" />
                </button>
              </div>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4">
              {messages.length === 0 ? <div className="text-center space-y-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex flex-wrap gap-2 justify-center">
                      {quickQuestions.map((q, i) => <button key={i} onClick={() => {
                  setInputValue(q);
                  inputRef.current?.focus();
                ...
                </div> : messages.map((message, index) => <div key={message.id} className="space-y-1">
                    <div className={`flex gap-3 ${message.role === 'user' ? 'flex-row-reverse' : ''}`}>
                      ...
                      <div className={`max-w-[80%] p-3 rounded-2xl text-sm ${message.role === 'user' ? 'bg-primary text-primary-foreground rounded-tr-md' : 'bg-muted text-foreground rounded-tl-md'}`}>
                        <p className="whitespace-pre-wrap">{message.content}</p>
                      </div>
                    </div>
                    
                    {/* Feedback buttons for assistant messages */}
                    {message.role === 'assistant' && !isLoading && <div className="flex items-center gap-1 ml-11">
                        {message.feedbackGiven ? <span className="text-xs text-muted-foreground">
                            {message.feedbackGiven === 'positive' ? '👍 Thanks!' : '👎 Thanks for letting us know'}
                          </span> : <>
                            <button onClick={() => handleFeedback(index, true)} className="p-1 rounded hover:bg-muted transition-colors text-muted-foreground hover:text-green-600" title="Helpful response">
                              <ThumbsUp className="w-3.5 h-3.5" />
                            </button>
                            <button onClick={() => handleFeedback(index, false)} className="p-1 rounded hover:bg-muted transition-colors text-muted-foreground hover:text-red-500" title="Not helpful">
            ...
            <div className="p-4 border-t border-border shrink-0">
              <div className="flex gap-2">
                <input ref={inputRef} type="text" value={inputValue} onChange={e => setInputValue(e.target.value)} onKeyPress={handleKeyPress} placeholder="Type your message..." className="flex-1 bg-muted border-0 rounded-full px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary" disabled={isLoading} />
                <Button onClick={handleSend} disabled={!inputValue.trim() || isLoading} size="icon" className="rounded-full w-10 h-10 shrink-0">
